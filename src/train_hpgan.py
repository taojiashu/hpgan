...
def main(args):
    ...
    d_inputs = tf.placeholder(dtype=tf.float32,
                              shape=[None, sequence_length] + list(train_data_reader.element_shape),
                              name="d_inputs")
    g_inputs = tf.placeholder(dtype=tf.float32,
                              shape=[None, input_sequence_length] + list(train_data_reader.element_shape),
                              name="g_inputs")
    ...
    g_first_frame_continuity_loss = tf.norm(g_next[:, 0:1, :, :] - g_prev[:, 0:1, :, :], ord=2)
    tf.summary.scalar("continuity_loss", g_first_frame_continuity_loss)
    ...
    g_loss = g_gan_loss + 0.001 * g_consistency_loss + 0.01 * g_bone_loss + 0.01 * g_first_frame_continuity_loss
        ...
        while epoch < max_epochs:
            ...
            while train_data_reader.has_more():
                ...
                if k == 0:
                    ...
                    skeleton2D.draw_to_file(skeleton_data, subject_id,
                                            os.path.join(output_folder, "pred_{}.png".format(epoch)))

                z_data = nn.generate_random(z_rand_type, z_rand_params, shape=[minibatch_size, z_size])
                for _ in range(int(d_per_mb_iterations - 1)):
                    _, d_loss_val = sess.run([d_op, d_loss],
                                             feed_dict={d_inputs: input_data, g_inputs: input_past_data, g_z: z_data})

                _, d_loss_val_prob = sess.run([d_op_prob, d_loss_prob],
                                              feed_dict={d_inputs: input_data, g_inputs: input_past_data, g_z: z_data})

                for _ in range(int(g_per_mb_iterations - 1)):
                    _, g_loss_val = sess.run([g_op, g_loss],
                                             feed_dict={g_inputs: input_past_data, d_inputs: input_data, g_z: z_data})

                summary, _, g_loss_val = sess.run([summary_op, g_op, g_loss],
                                                  feed_dict={g_inputs: input_past_data, d_inputs: input_data,
                                                             g_z: z_data})
                ...
                if save_model_and_video:
                    ...
                    if args.record_clip:
                        for sequence in skeleton_data:
                            skeleton2D.draw_to_video_file(sequence, os.path.join(output_videos_folder,
                                                                                 "pred_{}_z{}.mp4".format(epoch,
                                                                                                          video_index)))
                            video_index += 1
            ...
            if epoch > 20:
                ...
                logging.info(
                    "  generative best pos loss:\t{:e}, for epoch {}".format(g_best_pos_loss, g_best_pos_epoch))
